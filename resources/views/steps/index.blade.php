...
Stations d’Epuration des eaux usées (STEP)
	...
	<table class="table table-striped table-bordered table-hover">
		...
		<tbody>
			...
			<tr>
				...
				<td>
					{{$step->mise_en_demeure}}
				</td>
				<td>
					{{$step->annee_mis_en_dem}}
				</td>
			...
			<td style="width:50px">
				<a href="#" data-href="step/{{$step->id}}/edit" data-toggle="modal" data-target="#ModalMod" class="btn btn-link">
					<span class="glyphicon glyphicon-pencil" aria-hidden="true"></span>
				</a>
			</td>

			<td>
				<form action="step/{{$step->id}}"  method="POST" id="{{$step->id}}">
					<input type="hidden" name="_method" value="DELETE">
					<input type="hidden" name="_token" value="{{ csrf_token() }}">
					<button class="btn btn-link confirm-delete" type="submit" data-id="{{$step->id}}">
...
<div class="col-md-3">
	<div class="dataTables_info" role="status" aria-live="polite">
		<a href="step/create" class="btn btn-success btn-lg">
			Ajouter
		</a>
	</div>
</div>
<div class="col-md-3">
	<div class="dataTables_info" role="status" aria-live="polite">
		<h4>Affichage de {{$steps->count()}} enregistrements sur un total de {{$steps->total()}}</h4>
	</div>
</div>
<div class="col-md-6">
