...
<div class="col-md-12">
	<table class="table table-striped table-bordered table-hover">
		...
		<tbody>
			@foreach($contdigbas as $contdigba)
			<tr>
				<td>
					{{$contdigba->nom_contdigba}}
				</td>
			...
			<td style="width:50px">
				<a href="#" data-href="contdigba/{{$contdigba->id}}/edit" data-toggle="modal" data-target="#ModalMod" class="btn btn-link">
					<span class="glyphicon glyphicon-pencil" aria-hidden="true"></span>
				</a>
			</td>

			<td>
				<form action="contdigba/{{$contdigba->id}}"  method="POST" id="{{$contdigba->id}}">
					<input type="hidden" name="_method" value="DELETE">
					<input type="hidden" name="_token" value="{{ csrf_token() }}">
					<button class="btn btn-link confirm-delete" type="submit" data-id="{{$contdigba->id}}">
...
<div class="col-md-3">
	<div class="dataTables_info" role="status" aria-live="polite">
		<a href="contdigba/create" class="btn btn-success btn-lg">
			Ajouter
		</a>
	</div>
</div>
<div class="col-md-3">
	<div class="dataTables_info" role="status" aria-live="polite">
		<h4>Affichage de {{$contdigbas->count()}} enregistrements sur un total de {{$contdigbas->total()}}</h4>
	</div>
</div>
<div class="col-md-6">
