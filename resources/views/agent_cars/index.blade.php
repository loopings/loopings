...
@section('title')
Agent responsable des risques RR
@endsection
@section('subtitle')
...
<div class="col-md-12">
	<table class="table table-striped table-bordered table-hover">
		<thead>
			<tr>
				<th>Nom de l'agent RR</th>
				<th>Prenom de l'agent RR</th>
		...
		<tbody>
			...
			<td style="width:50px">
				<a href="#" data-href="agent_car/{{$agent_car->id}}/edit" data-toggle="modal" data-target="#ModalMod" class="btn btn-link">
					<span class="glyphicon glyphicon-pencil" aria-hidden="true"></span>
				</a>
			</td>

			<td>
				<form action="agent_car/{{$agent_car->id}}"  method="POST" id="{{$agent_car->id}}">
					<input type="hidden" name="_method" value="DELETE">
					<input type="hidden" name="_token" value="{{ csrf_token() }}">
					<button class="btn btn-link confirm-delete" type="submit" data-id="{{$agent_car->id}}">
