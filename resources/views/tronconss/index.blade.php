	...
	<table class="table table-striped table-bordered table-hover">
		<thead>
			<tr>
				...
				<th>
					Type (routier ou de voies ferrées )
				</th>
				<th>
		...
		<tbody>
			...
			<td style="width:50px">
				<a href="#" data-href="troncons/{{$troncons->id}}/edit" data-toggle="modal" data-target="#ModalMod" class="btn btn-link">
					<span class="glyphicon glyphicon-pencil" aria-hidden="true"></span>
				</a>
			</td>

			<td>
				<form action="troncons/{{$troncons->id}}"  method="POST" id="{{$troncons->id}}">
					<input type="hidden" name="_method" value="DELETE">
					<input type="hidden" name="_token" value="{{ csrf_token() }}">
					<button class="btn btn-link confirm-delete" type="submit" data-id="{{$troncons->id}}">
...
<div class="col-md-3">
	<div class="dataTables_info" role="status" aria-live="polite">
		<a href="troncons/create" class="btn btn-success btn-lg">
			Ajouter
		</a>
	</div>
</div>
<div class="col-md-3">
	<div class="dataTables_info" role="status" aria-live="polite">
		<h4>Affichage de {{$tronconss->count()}} enregistrements sur un total de {{$tronconss->total()}}</h4>
	</div>
</div>
<div class="col-md-6">
