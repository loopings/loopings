	...
	<table class="table table-striped table-bordered table-hover">
		...
		<tbody>
			...
			<tr>
				...
				<td>
					{{$plui->correspondant_ddt}}
				</td>
				<td>
					{{$plui->etat_avancement_plui}}
				</td>
			...
			<td style="width:50px">
				<a href="#" data-href="plui/{{$plui->id}}/edit" data-toggle="modal" data-target="#ModalMod" class="btn btn-link">
					<span class="glyphicon glyphicon-pencil" aria-hidden="true"></span>
				</a>
			</td>

			<td>
				<form action="plui/{{$plui->id}}"  method="POST" id="{{$plui->id}}">
					<input type="hidden" name="_method" value="DELETE">
					<input type="hidden" name="_token" value="{{ csrf_token() }}">
					<button class="btn btn-link confirm-delete" type="submit" data-id="{{$plui->id}}">
...
<div class="col-md-3">
	<div class="dataTables_info" role="status" aria-live="polite">
		<a href="plui/create" class="btn btn-success btn-lg">
			Ajouter
		</a>
	</div>
</div>
<div class="col-md-3">
	<div class="dataTables_info" role="status" aria-live="polite">
		<h4>Affichage de {{$pluis->count()}} enregistrements sur un total de {{$pluis->total()}}</h4>
	</div>
</div>
<div class="col-md-6">
