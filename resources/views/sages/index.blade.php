	...
	<table class="table table-striped table-bordered table-hover">
		<thead>
			<tr>
				...
				<th>
					Adresse internet de la fiche descriptive du SAGE
				</th>
				<th>
					Contact 1
				</th>
		...
		<tbody>
			...
			<td style="width:50px">
				<a href="#" data-href="sage/{{$sage->id}}/edit" data-toggle="modal" data-target="#ModalMod" class="btn btn-link">
					<span class="glyphicon glyphicon-pencil" aria-hidden="true"></span>
				</a>
			</td>

			<td>
				<form action="sage/{{$sage->id}}"  method="POST" id="{{$sage->id}}">
					<input type="hidden" name="_method" value="DELETE">
					<input type="hidden" name="_token" value="{{ csrf_token() }}">
					<button class="btn btn-link confirm-delete" type="submit" data-id="{{$sage->id}}">
...
<div class="col-md-3">
	<div class="dataTables_info" role="status" aria-live="polite">
		<a href="sage/create" class="btn btn-success btn-lg">
			Ajouter
		</a>
	</div>
</div>
<div class="col-md-3">
	<div class="dataTables_info" role="status" aria-live="polite">
		<h4>Affichage de {{$sages->count()}} enregistrements sur un total de {{$sages->total()}}</h4>
	</div>
</div>
<div class="col-md-6">
